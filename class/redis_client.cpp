...
namespace RedisWrap {
    ...
    int RedisClient::Set(int64_t key, const string &value)
    {
        ...
        reply = (redisReply*)ExcuteCommand("SET %lld %b", key, value.c_str(), value.size());
    ...
    int RedisClient::Get(const string &key, redisReply** reply)
    {
        int type = REDIS_ERR;
    ...
    int RedisClient::Get(int64_t key, redisReply** reply)
    {
        int type = REDIS_ERR;
        
        *reply = (redisReply*)ExcuteCommand("GET %lld", key);
    ...
    int RedisClient::HSet(int64_t key, int64_t filed, const string &value)
    {
        ...
        reply = (redisReply*)ExcuteCommand("HSET %lld %lld %b", key, filed, value.c_str(), value.size());
    ...
    int RedisClient::HGet(const string &key, const string &filed, redisReply** reply)
    {
        int type = REDIS_ERR;
